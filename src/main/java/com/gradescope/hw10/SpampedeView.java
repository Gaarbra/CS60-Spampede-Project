...
class SpampedeView {
  ...
  public SpampedeView(SpampedeController controller, SpampedeModel model, int width, int height) {
    ...
    this.frame = new JFrame("Spampede"); 
  ...
  public void updateGraphics() {
    ...
    // CALCULATE OFFSETS
    // Calculate the total pixel width of the grid (Columns * Size of one cell)
    int boardWidthInPixels = this.model.getNumColumns() * Preferences.CELL_SIZE;
    
    // Calculate how much space is left over, and divide by 2 to center it
    int xOffset = (this.width - boardWidthInPixels) / 2;
    int yOffset = 90;
    // Draw the board
    // The method drawSquare (below) will likely be helpful :)

    for (int row = 0; row < this.model.getNumRows(); row++){
        for (int col = 0; col < this.model.getNumColumns(); col++){
            Color c = this.model.getCellColor(row, col);

            int x = col * Preferences.CELL_SIZE + xOffset;
            int y = row * Preferences.CELL_SIZE + yOffset;

            this.drawSquare(x, y, c);
        }
    }
